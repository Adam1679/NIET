...
def parse_args():
    ...
    args.add_argument("-data", "--data",
                      default="./data/YAGO/", help="data directory")
    ...
    args.add_argument("-en_emb_size", "--entity_embedding_size", type=int,
                      default=120, help="Size of entity embeddings (if pretrained not used)")
    ...
    args.add_argument("-l", "--lr", type=float, default=0.01)
    ...
    args.add_argument("-outfolder", "--output_folder",
                      default="./checkpoints/yago/out/", help="Folder name to save the models.")

    # arguments for e2tGAT
    args.add_argument("-b_gat", "--batch_size_gat", type=int,
                      default=30000, help="Batch size for GAT")
    ...
    args.add_argument("-out_dim_long", "--out_long_dim", type=int,
                      default=120, help="Longer output embedding dimensions")
...
logger.addHandler(logging.FileHandler(filename="./connect_e_yago.log", mode='a'))
...
def train_ConnectE():
    ...
    optimizer = torch.optim.Adagrad (connectE2T_TRT.parameters (), lr=args.lr, weight_decay=args.weight_decay_gat)
    ...
    for epoch in range (args.epochs_gat) :
        ...
        if len (e2t_Corpus_.train_indices) % args.batch_size_gat == 0 :
            num_iters_per_epoch = len (e2t_Corpus_.train_indices) // args.batch_size_gat
        else :
        ...
        if len (trt_Corpus_.train_indices) % args.batch_size_gat == 0 :
            num_iters_per_epoch = len (trt_Corpus_.train_indices) // args.batch_size_gat
        else :
            num_iters_per_epoch = (len (trt_Corpus_.train_indices) // args.batch_size_gat) + 1
...
def evaluate_fscore():
    ...
    all_e2t = e2t_Corpus_.test_triples + e2t_Corpus_.validation_triples + e2t_Corpus_.train_triples
    evaluator1 = Type_Evaluator(test_e2t, all_e2t, logger)
    classification_evaluator = Classification_Evaluator_E2T(valid_e2t, test_e2t, all_e2t, train_trt, model, logger=logger,
                                                            save_path="/home/ubuntu/RNET_gpu/checkpoints/yago/out/classification_result",
                                                            save_data=True)
