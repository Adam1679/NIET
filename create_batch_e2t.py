...
# -*- coding: utf-8 -*-
"""
Created on Sat Sep  5 23:05:50 2020
...
class Corpus_e2t:
    def __init__(self, args, train_data, validation_data, test_data, entity2id,
                 type2id,  batch_size, valid_to_invalid_samples_ratio, unique_entities_train, unique_types_train, get_2hop=False):
        ...
        self.unique_types_train = [self.type2id[i]
                                      for i in unique_types_train]
    ...
    def get_iteration_batch(self, iter_num):
        if (iter_num + 1) * self.batch_size <= len(self.train_indices):
            ...
            if self.invalid_valid_ratio > 0:
                ...
                random_types = np.random.randint(0, len(self.type2id), last_index * self.invalid_valid_ratio)
                ...
                for i in range(last_index):
                    ...
                    for j in range(self.invalid_valid_ratio // 2):
                        ...
                        while (self.batch_indices[last_index + current_index, 0], self.batch_indices[last_index + current_index, 1],
                               random_types[current_index]) in self.valid_triples_dict.keys():
                            random_types[current_index] = np.random.randint(0, len(self.type2id))
                        self.batch_indices[last_index + current_index,
                                           2] = random_types[current_index]
                        self.batch_values[last_index + current_index, :] = [-1]
            ...
            if self.invalid_valid_ratio > 0:
                ...
                # Precopying the same valid indices from 0 to batch_size to rest
                # of the indices
                self.batch_indices[last_index:(last_index * (self.invalid_valid_ratio + 1)), :] = \
                    np.tile(self.batch_indices[:last_index, :], (self.invalid_valid_ratio, 1))
                self.batch_values[last_index:(last_index * (self.invalid_valid_ratio + 1)), :] = \
                    np.tile(self.batch_values[:last_index, :], (self.invalid_valid_ratio, 1))
                ...
                for i in range(last_index):
                    ...
                    for j in range(self.invalid_valid_ratio // 2):
                        ...
                        while (self.batch_indices[last_index + current_index, 0], self.batch_indices[last_index + current_index, 1],
                               random_types[current_index]) in self.valid_triples_dict.keys():
                            random_types[current_index] = np.random.randint(0, len(self.type2id))
                        self.batch_indices[last_index + current_index, 2] = random_types[current_index]
                        self.batch_values[last_index + current_index, :] = [-1]
