...
from models import SpKBGATModified, multiRSGAT, SpKBGATConvOnly
...
logger.addHandler(logging.FileHandler(filename="./rngat.log", mode='w'))
...
def parse_args():
    ...
    args.add_argument("-l", "--lr", type=float, default=3e-4)
    ...
    args.add_argument("-drop_GAT", "--drop_GAT", type=float, 
                      default=0.2, help="Dropout probability for SpGAT layer")
...
def train_gat(args):
    ...
    model_rsgat.load_state_dict(torch.load("./checkpoints/fb/out/rngat.trained_4999.pth"))
    start = 5000
    ...
    epoch_losses_ere = []
    epoch_losses_trt = []
    logger.info("Number of epochs {}".format(args.epochs_gat))
    ...
    for epoch in range(start, start+args.epochs_gat):
        ...
        for iters in range(num_iters_per_epoch):
            ...
            loss_e2t = batch_gat_loss_e2t(gat_loss_func, train_indices_e2t, out_entity_embed_short, output_rdf_embedding, out_type_embed)
            loss_ere = batch_gat_loss(gat_loss_func, train_indices_ere, out_entity_embed_long, out_relation_embed_long)
            loss_trt = batch_gat_loss(gat_loss_func, train_indices_trt, out_type_embed, out_relation_embed_short)

            final_loss = 0.4*loss_e2t + 0.4*loss_ere + 0.2*loss_trt
            final_loss.backward()
            optimizer.step()
        ...
        # scheduler.step()
        logger.info("Epoch {} , average loss e2t {} , average loss ere {} , average loss trt {} , epoch_time {}".format(
