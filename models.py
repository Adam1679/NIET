...
class multiRSGAT(nn.Module):
    def __init__(self, input_entity_embeddings, input_relation_embeddings, input_type_embeddings, input_rdf_relation_embeddings, args):
        ...
        self.output_entity_embeddings_short = nn.Parameter(torch.randn(self.num_entities, self.out_short_dim))
        self.output_entity_embeddings_long = nn.Parameter(torch.randn(self.num_entities, self.out_long_dim))
        self.output_relation_embeddings_short = nn.Parameter(torch.randn(self.num_relation, self.out_short_dim))
        self.output_relation_embeddings_long = nn.Parameter(torch.randn(self.num_relation, self.out_long_dim))
        self.output_type_embedding = nn.Parameter(torch.randn(self.num_types, self.out_short_dim))
        self.output_rdf_embedding = nn.Parameter(torch.randn(self.num_rdf_relation, self.out_short_dim))
        
        self.model_gat_e2t = SpKBGATModified_e2t(self.input_entity_embeddings, 
                                                 self.input_rdf_relation_embeddings, 
                                                 self.input_type_embeddings, 
                                                 self.out_short_dim, 
                                                 self.out_short_dim, 
                                                 self.out_short_dim, 
                                                 self.drop_GAT, 
        ...
        self.model_gat_ere = SpKBGATModified(self.input_entity_embeddings, 
                                             self.input_relation_embeddings, 
                                             self.out_long_dim, 
                                             self.out_long_dim,
                                             self.drop_GAT, 
                                             self.alpha, 
    ...
    def forward(self, e2t_Corpus_, ere_Corpus_, trt_Corpus_, train_indices_e2t, train_indices_ere, train_indices_trt):
        entity_embed_e2t, type_embed_e2t, rdf_relation_embed_e2t = self.model_gat_e2t(e2t_Corpus_, e2t_Corpus_.train_adj_matrix, train_indices_e2t)
        entity_embed_ere, relation_embed_ere = self.model_gat_ere(ere_Corpus_, ere_Corpus_.train_adj_matrix, train_indices_ere)
        type_embed_trt, relation_embed_trt = self.model_gat_trt(trt_Corpus_, trt_Corpus_.train_adj_matrix, train_indices_trt)
        
        union_entity_embed = torch.cat([entity_embed_e2t, entity_embed_ere], dim=1)
        union_type_embed = torch.cat([type_embed_e2t, type_embed_trt], dim=1)
        union_relation_embed = torch.cat([relation_embed_ere, relation_embed_trt], dim=1)
        ...
        self.output_entity_embeddings_long.data = out_entity_embed_long.data
        self.output_relation_embeddings_short.data = out_relation_embed_short.data
        self.output_relation_embeddings_long.data = out_relation_embed_long.data
        self.output_type_embedding.data = out_type_embed.data
        self.output_rdf_embedding.data = out_rdf_embed.data

        return out_entity_embed_long, out_entity_embed_short, out_relation_embed_long, out_relation_embed_short, out_type_embed, out_rdf_embed
