...
# -*- coding: utf-8 -*-
"""
Created on Sat Oct 10 12:19:37 2020
...
class multiRSGAT(nn.Module):
    def __init__(self, input_entity_embeddings, input_relation_embeddings, input_type_embeddings, input_rdf_relation_embeddings, args):
        ...
        self.input_rdf_relation_embeddings = nn.Parameter(input_rdf_relation_embeddings, requires_grad=False)        
        self.input_rdf_relation_embeddings.fill_(0)
        self.output_entity_embeddings_short = nn.Parameter(
            torch.randn(self.num_entities, self.out_short_dim))
        self.output_entity_embeddings_long = nn.Parameter(
            torch.randn(self.num_entities, self.out_long_dim))
        self.output_relation_embeddings_short = nn.Parameter(
            torch.randn(self.num_relation, self.out_short_dim))
        self.output_relation_embeddings_long = nn.Parameter(
            torch.randn(self.num_relation, self.out_long_dim))
        self.output_type_embedding = nn.Parameter(
            torch.randn(self.num_types, self.out_short_dim))
        self.output_rdf_embedding = nn.Parameter(
            torch.randn(self.num_rdf_relation, self.out_short_dim))
...
#        self.linear_r2short = nn.Linear(in_features = self.nheads_GAT[0]*(self.out_long_dim + self.out_short_dim), out_features = self.out_short_dim)
        ...
        #     size=(self.out_long_dim, self.out_short_dim)))
        # nn.init.xavier_uniform_(self.M.data, gain=1.414)
        self.entity2short = nn.Linear(self.out_long_dim, self.out_short_dim)
        self.relation2short = nn.Linear(self.out_long_dim, self.out_short_dim)
    ...
    def forward(self, e2t_Corpus_, ere_Corpus_, trt_Corpus_, train_indices_e2t, train_indices_ere, train_indices_trt):
        ...
        #print('here:', 
        #      entity_embed_e2t.size(), type_embed_e2t.size(), rdf_relation_embed_e2t.size(),
        #      '\n', entity_embed_ere.size(), relation_embed_ere.size(),
        #      '\n', type_embed_trt.size(),relation_embed_trt.size())
        ...
        out_entity_embed_short = F.relu(self.entity2short(out_entity_embed_long))
        out_relation_embed_short = F.relu(self.relation2short(out_relation_embed_long))
